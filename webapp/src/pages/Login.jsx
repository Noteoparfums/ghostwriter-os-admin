  ...
  const handleLogin = async (e) => {
    ...
    if (isSignUp) {
      ...
      if (signUpError) {
        ...
        // Try to create a profile row client-side
        if (data?.user) {
      ...
      <main className="relative z-10 w-full max-w-md px-4 sm:px-gutter">
        <div className="glass-panel rounded-xl p-6 sm:p-8 shadow-[0_0_40px_-10px_rgba(207,188,255,0.15)] flex flex-col items-center w-full transition-all duration-300 bg-surface-container-lowest/40 backdrop-blur-xl border border-white/10">
          ...
          <div className="mb-8 text-center">
            ...
            <p className="font-body-base text-sm text-on-surface-variant">Quiet Luxury for Power Users.</p>
          </div>
          {/* Email/Password Authentication */}
          <form className="w-full mb-7" onSubmit={handleLogin}>
            ...
            <div className="text-center mt-3">
              <button
                ...
                className="text-primary/70 hover:text-primary font-label-sm text-xs transition-colors"
              >
          ...
          <div className="w-full flex flex-col gap-3">
            <button onClick={() => handleOAuthLogin('google')} className="w-full bg-transparent border border-white/10 hover:border-primary/50 text-on-surface font-body-base text-sm py-2.5 px-4 rounded-lg flex items-center justify-center gap-3 transition-all duration-300 hover:bg-white/5 hover:scale-[1.02]">
            ...
            <button onClick={() => handleOAuthLogin('github')} className="w-full bg-transparent border border-white/10 hover:border-primary/50 text-on-surface font-body-base text-sm py-2.5 px-4 rounded-lg flex items-center justify-center gap-3 transition-all duration-300 hover:bg-white/5 hover:scale-[1.02]">
